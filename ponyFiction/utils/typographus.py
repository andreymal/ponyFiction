...
def entity(name):
    return chr(htmlentitydefs.name2codepoint[name])
...
#Предлоги и союзы
prepos = 'а|в|во|вне|и|или|к|о|с|у|о|со|об|обо|от|ото|то|на|не|ни|но|из|изо|за|уж|на|по|под' \
    + '|подо|пред|предо|про|над|надо|как|без|безо|что|да|для|до|там|ещё|их|или|ко|меж|между' \
    + '|перед|передо|около|через|сквозь|для|при|я'

metrics = 'мм|см|м|км|кг|б|кб|мб|гб|dpi|px' # с граммами сложнее - либо граммы либо города
...
any_quote = '(?:%s)' % ('|'.join(map(unicodedata.lookup, known_quotes)))
...
class Rule:
    ...
    def __repr__(self):
        return ("re.compile('%s', %s).sub('%s')" % (self.pattern, self.flag, self.replacement)).encode('utf-8')
...
#             i+=1
#             rep = '%s\g<1>%s\g<2>%s' % (q['lq'], sym['lquote2'], sym['rquote2'])
