...
def stream_list(request, **kwargs):
    ...
    if stream_type == 'comments':
        ...
        object_list = model.objects.filter(draft=False,approved=True).order_by('-date')[0:settings.STORIES_COUNT['stream']]
        ...
        object_list = model.objects.filter(draft=False,approved=False).order_by('-date')[0:settings.CHAPTERS_COUNT['stream']]
