...
class stories_rss(Feed):
    title = 'Новые истории - stories.everypony.ru'
    link = '/new/stories/'
...
class chapters_rss(Feed):
    title = 'Обновления глав - stories.everypony.ru'
    link = '/new/chapters/'
    ...
    def item_link(self, item):
        return "/story/%i/chapter/%i/" % (item.in_story_id, item.order)
...
class story_chapters_rss(Feed):
    ...
    def items(self, obj):
        return Chapter.objects.filter(in_story_id=obj.id).order_by('-order')[:settings.RSS['chapters']]
